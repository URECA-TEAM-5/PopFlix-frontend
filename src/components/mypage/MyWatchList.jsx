    ...
    <OpacityDiv>
      ...
      <WatchListContainer>
        {isLoading ? (
              ...
              <WatchListItem>
                ...
                <div className="itemList">
                  ...
                  {list.movies.length > 0 ? (
                    <Link to={`/watchlist/${list.id}`} className="storageName point">
                      {list.storageName}
                    </Link>
                  ) : (
                  ...
                  <ToggleDiv>
                    <span>공개여부</span>
                    <input role="switch" type="checkbox" checked={list.isPublic} onChange={() => handleTogglePublic(list.id, list.isPublic, list.movieCount)} />
                    <Link to={`/watchlist/${list.id}/edit`} className="linkPoint point">
                      편집
                    </Link>
                    <span className={`point ${list.movieCount === 0 ? 'disabled' : ''}`} onClick={() => handleCopy(list.id, list.isPublic)}>
                      공유
                    </span>
                    <span className="point" onClick={() => handleDelete(list.id)}>
                      삭제
                    </span>
                  </ToggleDiv>
                </div>
          ...
          <EmptyResult img="/assets/popcorn_score_null.svg" message="New Folder를 눌러서 WatchList를 추가해주세요!" size="4" fontSize="1.1" />
        )}
      </WatchListContainer>
